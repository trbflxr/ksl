...
More info on [config creation](#iconfig) and [custom type parsers](#typeparsert) below.

> [!NOTE]  
> Also consider checking out an [example KSL mod](https://github.com/trbflxr/ksl_sdk) to get a more detailed example.

This is an advanced way to save the mod / extension config values. If you need a simple way to store mod / extension data, consider using [Prefs storage](https://github.com/trbflxr/ksl/blob/master/doc/api/prefs.md).
...
* Unity.Vector3
* Unity.Rect
* Support for [custom types](#registerparser)
...
Use this method if config(s) you want to register contain(s) custom types that are not listed in [types list](#supported-types).

More info and how-to-use examples [here](#custom-parser-registration).
...
Keep in mind that if a config contains custom types which are not listed in [types list](#supported-types) you have to call [RegisterParser](#registerparser) and register a parser for it.

More [info and examples](#config-registration) further down.
...
Use this method for marking config of type **T** for save. The config will be written on the disc with slight delay so it is preferred to use in something like [sliders](ui.md#slider). Mainly is to be used after changing custom type values: [example](#config-auto-and-manual-save).
...
Use this method to trigger force save for config of type **T** without any delays. Mainly is to be used after changing custom type values: [example](#config-auto-and-manual-save).
...
Method for converting custom type to a string. See [example](#custom-type-parser-example) below.
...
Method for parsing custom type from string. See [example](#custom-type-parser-example) below.
...
Config has to be defined as a public **interface** and registered using [RegisterConfig](#registerconfig) which will return an instance of a config that can be stored.

List of [supported types](#supported-types). Also you can define [custom types](#custom-type-parser-example) and register parsers using [RegisterParser](#registerparser).

Registered configs have an auto-save functionality **only** for types from the [list](#supported-types). [ForceSave](#forcesave) custom types should be called after the value change. More about it [here](#config-auto-and-manual-save).
...
Example of a [CustomType](#custom-type-parser-example) and a parser for it.

### Custom parser registration

If the config contains custom types which are not presented in the [supported types](#supported-types) list parser registration should be done first.
Example of a custom type and a type parser can be found [here](#custom-parser-registration).
...
Keep in mind that if the config contains types that not presented in the [supported types](#supported-types) list, [parser registration](#custom-parser-registration) should be done first.

After registration config should be stored for future use.

Learn more about [config save system](#config-auto-and-manual-save).
...
A config can be registered without setting the name using [RegisterConfig](#registerconfig), but you can only do it once. The config file will be named the same as the mod.
...
For all config properties with a [supported type](#supported-types) KSL will add a listener and on any property change the config will be marked for save.

If the config has [custom types](#custom-type-parser-example) properties it will only be marked for save when setting a property value directly (example below). But in a case of setting a value to one of custom type's properties config will not be marked for save.
